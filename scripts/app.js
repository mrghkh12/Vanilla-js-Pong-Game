...
function handleLose() {
  ...
  if (
    gamePoint <= computerScore.innerHTML ||
    gamePoint <= playerScore.innerHTML
  ) {
    isFinish = true;
    ...
    if (playerScore.innerHTML > computerScore.innerHTML) {
      difficultyWrapper.innerHTML =
        '<h2>Congratulations, you were able to <span style="color: rgb(79, 202, 79);">Win🎉</span></h2>';
    } else {
      difficultyWrapper.innerHTML =
        '<h2>Sorry, you <span style="color: red;">lost😔</span></h2>';
    }
  }
}
...
function changeBgColor(delta) {
  let hue = parseFloat(
    getComputedStyle($.documentElement).getPropertyValue("--hue")
  );
...
difficultyWrapper.addEventListener("click", (e) => {
  if (e.target.tagName === "DIV") {
    for (let item of difficultyWrapper.children)
